"""Fuse MediaPipe pose landmarks with aligned depth + CameraInfo for 3D upper leg pose."""
...
def _build_rotation_x_forward(x_dir: np.ndarray) -> np.ndarray:
    """Right-handed R; columns are upper-leg-frame axes in camera frame. +x along bone (distal - proximal)."""
...
class UpperLegPose3DNode(Node):
    def __init__(self) -> None:
        super().__init__('upperleg_pose_3d_node')

        self.declare_parameter('landmarks_topic', 'pose_landmarks')
        self.declare_parameter('depth_topic', '/camera/camera/aligned_depth_to_color/image_raw')
        self.declare_parameter('camera_info_topic', '/camera/camera/color/camera_info')
        self.declare_parameter('output_pose_topic', 'upperleg_pose_camera')
        self.declare_parameter('approx_sync_slop_sec', 0.08)
        self.declare_parameter('sync_queue_size', 50)
        self.declare_parameter('landmark_proximal', 24)
        self.declare_parameter('landmark_distal', 26)
    ...
    def _on_sync(self, lm_msg: PoseLandmarksStamped, depth_msg: Image) -> None:
        ...
        p24 = self._deproject(uda, vda, za)
        p26 = self._deproject(udb, vdb, zb)
        pos = 0.5 * (p24 + p26)
        bone = p26 - p24
